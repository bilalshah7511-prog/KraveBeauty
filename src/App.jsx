...
export default function App() {
  ...
  useEffect(() => {
    if (!sizeMenuOpen && !countryMenuOpen && !stateMenuOpen) return undefined;
    function onPointerDown(e) {
      if (sizeMenuOpen && sizeSelectRef.current && !sizeSelectRef.current.contains(e.target)) {
        setSizeMenuOpen(false);
      }
      if (countryMenuOpen && countrySelectRef.current && !countrySelectRef.current.contains(e.target)) {
        setCountryMenuOpen(false);
      }
      if (stateMenuOpen && stateSelectRef.current && !stateSelectRef.current.contains(e.target)) {
  ...
  }, [sizeMenuOpen, countryMenuOpen, stateMenuOpen]);
    ...
    <>
      ...
      <header className="site-header">
        <div className="site-header__row">
          ...
          <a href="#" className="site-logo" aria-label="KraveBeauty">
            <img src={brand.logoSrc} alt="KraveBeauty" />
          </a>
      ...
      <div className={`mobile-menu${menuOpen ? ' is-open' : ''}`}>
        <div className="mobile-menu__body">
          ...
          <nav className="mobile-menu__nav">
            <a href="#" className="mobile-menu__nav-main">shop</a>
            <a href="#" className="mobile-menu__nav-main">routines</a>
                ...
                <div className="delivery-panel">
                  ...
                  <div className="country-select" ref={countrySelectRef}>
                    <div className={`country-select__wrap${countryMenuOpen ? ' is-open' : ''}`}>
                      <button
                        type="button"
                        className={`checkout-input country-select__input${!country ? ' is-placeholder' : ''}`}
                      ...
                      {countryMenuOpen && (
                        <ul className="country-select__menu" role="listbox">
                          {[
                            ...
                            <li key={c.value} role="presentation">
                              <button
                                ...
                                className={`country-select__option${country === c.value ? ' is-selected' : ''}`}
                                onClick={() => {
                                  setCountry(c.value);
                                  setCountryMenuOpen(false);
                                }}
                    ...
                    <div>
                      ...
                      <div className="sheet-select" ref={stateSelectRef}>
                        <div className={`sheet-select__wrap${stateMenuOpen ? ' is-open' : ''}`}>
                          <button
                            type="button"
                            className={`checkout-input sheet-select__input${!stateCode ? ' is-placeholder' : ''}`}
                            aria-haspopup="listbox"
                            aria-expanded={stateMenuOpen}
                            onClick={() => {
                              setCountryMenuOpen(false);
                              setStateMenuOpen((o) => !o);
                            }}
                          >
                            {usStates.find((s) => s.code === stateCode)?.name || 'Select state'}
                          </button>
                          {stateMenuOpen && (
                            <ul className="sheet-select__menu sheet-select__menu--states" role="listbox">
                              <li role="presentation">
                                <button
                                  type="button"
                                  role="option"
                                  aria-selected={!stateCode}
                                  className={`sheet-select__option sheet-select__option--placeholder${!stateCode ? ' is-selected' : ''}`}
                                  onClick={() => {
                                    setStateCode('');
                                    setStateMenuOpen(false);
                                  }}
      ...
      <StatusOverlay
        ...
        type="check"
        title="Order placed"
        text="Your KraveBeauty order is on the way."
        onDone={() => {
          setSuccessOpen(false);
          closeCart();
          setCompletion({ shipping: false, delivery: false, payment: false });
